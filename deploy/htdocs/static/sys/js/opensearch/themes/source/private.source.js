...
(function( win, S ){
    ...
    S.theme.priv = {
        ...
        getHtmlDescription: function( offerUrl, subject, src, type, isImgPrivate ){
			...
			//如果是privateoffer 加蒙版
			if (isImgPrivate){
				_html+='<div class="block"><p><a href="'+offerUrl+'"></a></p></div>'
			}
			_html+='<div class="vertical-img"><a class="a-img box-img" href="'+offerUrl+'" title="'+subject+'"><img src="'+S.Util.getOfferImgUrl( src, type)+'" alt="'+subject+'"/></a></div></dt><dd class="description"><a href="'+offerUrl+'" title="'+subject+'">'+subject+'</a></dd>'
            return _html;
        },
		/**
		 * get html dd.price
        ...
        getHtmlPrice: function( priceLabel, currency, value, isPricePrivate ){
            if ( isPricePrivate ){
                return '<dd class="price">'+priceLabel+'<span class="txt fd-cny">&yen; 授权可见</span></dd>';
            }
            var c = '';
            switch( currency ){
                case '元':
        ...
        getHtmlRoot: function( result, customize, config ){
			...
			//全局变量RESULTCOUNT 设置返回offer总数
			if (FYG('page-control')) this.getPageList(result.searchResultCount);
			//RESULTCOUNT=result.searchResultCount;
		...
		var pageAmount=Math.ceil(count/pageRecord.pageSize);
		pageRecord.pageAmount = pageAmount;
		$('page-amount').innerHTML =  pageRecord.pageAmount;
		var start = requestParam.beginPage - 4, end = 2 + Number(requestParam.beginPage), html = [] ;
		if(start < 1){
			end = end+1-start; // Translation the region
			start = 1;
			if(end > pageRecord.pageAmount){ // the end have overflow
				end = pageRecord.pageAmount;
			}
		}else if(end > pageRecord.pageAmount){ 
			start = start - end + pageRecord.pageAmount;
			end = pageRecord.pageAmount;
			if(start < 1){ // the start have overflow
				start = 1;
			}
		
		}
		
		if(requestParam.beginPage != 1){
			html.push('<li class="pre"><a href="#" class="fake-link">&lt;上一页</a> </li>');
		}else {
			html.push('<li class="pre disable"><a href="#" class="fake-link">&lt;上一页</a> </li>'); // disable the pre button
		}
		
		if(start != 1){
			html.push('<li class="omit">...</li>');
		}
		
		for(var i = start; i <= end; i++){
			if(requestParam.beginPage == i){
				html.push('<li class="current"><a href="#" class="fake-link">'+i+'</a></li>');
			}else{
				html.push('<li><a href="#" class="fake-link">'+i+'</a></li>');
			}
		}
		
		if(end != pageRecord.pageAmount){
			html.push('<li class="omit">...</li>');
		}
		
		if(requestParam.beginPage != end){
			html.push('<li class="next"><a href="#" class="fake-link">下一页&gt;</a></li>');
		}else{
			html.push('<li class="next disable"><a href="#" class="fake-link">下一页&gt;</a></li>'); // disable the next button
		}
		
		var html = html.join('');
		$$('#page-control ul')[0].innerHTML = html;
