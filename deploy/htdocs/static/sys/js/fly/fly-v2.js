...
(function($, Util, win) {
    ...
    $.extend(Fly.prototype, {
        ...
        _request: function() {
			...
			(self.config.apitype !== 'commend2') || (API = 'http://res.1688.com/fly/irecom.do');
			(self.config.apitype !== 'irecom') || (API = 'http://res.1688.com/fly/irecom.do');
			(self.config.apitype !== 'commend3') || (API = 'http://10.20.151.51:7001/fly/irecom.do');
			(self.config.apitype !== 'commend4') || (API = 'http://10.20.143.66:91/fly/irecom.do');
            for (i in params) {
     ...
     */
	Fly.getUid=function(){
		var query = win.location.search.substring(1).toLowerCase(),
		memberId = $.unparam(query, '&').memberid,
		lastLoginId = Util.lastLoginId,
		loginId = Util.loginId,
		beaconId = $.util.cookie('ali_beacon_id');
		return  memberId || loginId || lastLoginId || beaconId || -1;
	}
