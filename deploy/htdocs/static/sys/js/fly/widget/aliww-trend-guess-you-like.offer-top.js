...
(function(win,S){
	...
	L.augmentObject(S.RankTophot.prototype,{
		...
		_onFailure:function(){
			$('tab-trend').style.border = '0px'
			$('tab-trend').innerHTML = '<div style="height:200px; text-align:center; font-size:14px; font-weight:bold;"><p style="margin-top:100px; margin-bottom:8px;">获取商品失败</p><p>请<a href="#" title="刷新" class="refresh-page" onclick="javascript:window.location.reload();">刷新</a>页面重新尝试</p></div>';
		},
		
        _renderOfferTab:function(){
			for(var i=0,l=this.result.data.length; i<l;i++){
				$('tab-hot'+(i+1)).innerHTML = '<a href="#" title="'+this.result.data[i].categoryDesc+'">'+this.result.data[i].categoryDesc+'</a>';
			}
		},
		...
		_stripBadWord:function(str){
	     ...
	     _renderOfferTitle:function(offer){
			...
			var _title = '<span class="cont"><a href="'+detailUrl+'" title="'+detailSubject+'" onmousedown="FD.sys.fly.Utils.iClick({\'page\':'+this.oFlyConfig.coaseType+',\'objectId\':\''+offer.offerId+'\',\'recId\':\''+this.oFlyConfig.recid+'\',\'alg\':\''+offer.alg+'\',\'objectType\':\'offer\',\'pid\':\''+this.oFlyConfig.pid+'\'})">'+FD.sys.fly.Utils.doSubstring(detailSubject,18,'...')+'</a></span>';
			return _title;
		},
