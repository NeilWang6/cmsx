...
(function(win,S){
	 ...
	 */
	S.DetailErrorFocus = function(callback,data,oFlyConfig,oMergedFlyConfig){
		//实例化父类
		S.DetailErrorFocus.superclass.constructor.call(this, data,oFlyConfig,oMergedFlyConfig);
		this.result = data;
	...
	//继承父类
	L.extend(S.DetailErrorFocus,S.AbstractFlyView);
	//接口实例化
	L.augment(S.DetailErrorFocus,S.InterfaceFlyView);
	
	//方法封装
	L.augmentObject(S.DetailErrorFocus.prototype,{
		onSuccess:function(){
			FD.sys.fly.Utils.log('DetailErrorFocus');
			this._render();
		},
		onFailure:function(){
			this._failRender();
		...
		_failRender:function(){
            ...
            fHtml.push( this._failrenderFoot() );
        ...
        _renderOfferPrice:function(offer){
            ...
            var _ddPrice = price !== 0||'' ? '<dd class="price">' + currency + '<em class="value">' + price.toFixed(2) +'</em></dd>' : '<dd class="price red">价格面议</dd>';
            return _ddPrice;
        },
		_renderFoot:function(){
            var _modClose = '</ul>\n</div>\n</div>\n',
				_moreProductsUrl = FYG('moreProductsUrl').value;
			_modClose += '<div class="btn-box"><a class="btn-m" href="'+_moreProductsUrl+'" onmousedown="return aliclick(null, \'?tracelog=offererrormore\')">更多该供应商的产品信息&gt;&gt;</a></div>';
			return _modClose;
		},
		//取不到数据底部渲染
		_failrenderFoot:function(){
			var _modClose = '</ul>\n</div>\n</div>\n';
			return _modClose;
		},
		end:0
