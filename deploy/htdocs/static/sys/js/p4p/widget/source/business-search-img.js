...
(function(win,S){
	...
	S.P4POfferRecommendedRender=function(data,id,num){
		this.maxOfferNum = num || 6; //p4p广告位最多显示为3条
		this.data = data;
	...
	L.augmentObject(S.P4POfferRecommendedRender.prototype,{
		...
		_doRenderItem:function(item, idx){
	        ...
	        var itemHTML=''
	            +'<li class="window_offer" '+(idx+1==this.maxOfferNum?'style="margin-right:0;"':'')+'>'
	            +'<div class="imgbox_v2">'
	            +'<a onmousedown="FD.sys.p4p.Utils.p4pTagClick(\''+item.RESOURCEID+'\',\''+item.REDKEY+'\',\'819032_1008\');BB.aliclick(this,\'?searchtrace='+this.searchtrace+'\')" href="'+item.EURL+'" target="_blank"><img src="'+item.OFFERIMGURL+'"/></a>'
	            +'</div>'   
	            +'<h3 class="summary"><a onmousedown="FD.sys.p4p.Utils.p4pTagClick(\''+item.RESOURCEID+'\',\''+item.REDKEY+'\',\'819032_1008\');BB.aliclick(this,\'?searchtrace='+this.searchtrace+'\')" href="'+item.EURL+'" target="_blank">'+(function(){
					...
					if(lg>32){
						return U.doRed(U.doSubstringAo(U.doReplace(item.TITLE), 28, 2),item.REDKEY);
					}else{
						return U.doRed(item.TITLE,item.REDKEY);
					}
				})()+'</a></h3>'
	            +'<p class="price">'+this._doRenderPrice(item)+'<br/>'+this._doRenderQuantityBegin(item)+''
	    ...
	    _doRenderQuantityBegin:function(item){
			...
			if(item.OFFERPRICE==" "){
				...
				if(item.QUANTITYBEGIN.length>7){
					return '<b title="'+item.QUANTITYBEGIN+'">'+U.doSubstringAo(item.QUANTITYBEGIN,5) +'...'+item.PRICEUNIT+'起批</b>';
				}else{
	    ...
	    _doRenderPrice:function(item){
			...
			if(price.length==1){
				...
				if(p1.length==9){
					...
					return '<em title="'+price.join('.')+'">&yen;'+p1.substr(0,9)+'...</em>';
				}else{
	   ...
	   _doRenderWangWang:function(item,idx){
		...
		_doRenderContent:function(items){
	        ...
	        var output=''
	            +'<ul class="content_v2" id="J_p4p_items">'
	            +(function(t){
	                var itemsHTML='';
