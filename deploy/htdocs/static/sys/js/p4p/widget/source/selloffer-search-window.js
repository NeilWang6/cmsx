...
(function(win,S){
	...
	L.augmentObject(S.P4POfferRecommendedView.prototype,{
		onSuccess:function(){
			...
			new S.P4POfferRecommendedRender(this.result,'p4poffer',4);
		},
		onFailure:function(){
	...
	S.P4POfferRecommendedRender=function(data,id,num){
		this.maxOfferNum = num || 4; //p4p广告位最多显示为6条
		this.data = data;
		this.root = document.getElementById(id); //offer插入的位置标识
		if(!this.root)return;
		this.searchtrace = 'sale_search_p4phot_offer';
	    this.searchtrace_more = 'sale_search_p4phot_more';
		if(typeof window.iPageConfig=='object'&&window.iPageConfig.keywords==""){
			this.searchtrace = 'sale_list_p4phot_offer';
			this.searchtrace_more = 'sale_list_p4phot_more';
		}
	...
	L.augmentObject(S.P4POfferRecommendedRender.prototype,{
		...
		_doRenderHeader:function(){
			return '<div class="p4p-header"><h3>热门推荐</h3><a href="http://view.1688.com/cms/video/zhindex.html?tracelog=p4p_exhibition" target="_blank" class="p4p-m p4p-expo">热门展会推荐</a><a href="'+S.API_CONFIG.entrance+'" target="_blank" title="我也要出现在这里">我也要出现在这里</a></div>';
		},
		//拼装p4p广告位底部
		...
		_doRenderImage:function(item,idx){
			...
			if(item.OFFERIMGURL&&item.OFFERIMGURL!= ""){
				...
				_imgurl = _imgurl.replace('summ.jpg','search.jpg');
			}else {
				_imgurl = S.API_CONFIG.noimg.x150;
			}
			return '<div class="p4p-pic"><a href="'+item.EURL+'" title="'+FD.sys.p4p.Utils.doReplace(item.TITLE)+'" target="_blank" onmousedown="FD.sys.p4p.Utils.p4pTagClick(\''+item.RESOURCEID+'\',\''+item.REDKEY+'\',\'819032_1008\');SS.aliclick(this,\'?searchtrace='+this.searchtrace+'\')"><img src="'+_imgurl+'"/></a></div>';
		},
		//拼装标题元件
		_doRenderTitle:function(item){
			var U = FD.sys.p4p.Utils;
			return '<h3><a title="'+U.doReplace(item.TITLE)+'" href="' + item.EURL + '" onmousedown="FD.sys.p4p.Utils.p4pTagClick(\''+item.RESOURCEID+'\',\''+item.REDKEY+'\',\'819032_1008\');SS.aliclick(this,\'?searchtrace='+this.searchtrace+'\')" target="_blank" >' + U.doRed(U.doSubstringAo(U.doReplace(item.TITLE),20),item.REDKEY) + '</a></h3>';
		},
		//拼装公司名称元件
		...
		doRender:function(){
			...
			var html = [];
			html.push(this._doRenderHeader());
			html.push(this._doRenderContent(this.data));
			html.push(this._doRenderFooter());
