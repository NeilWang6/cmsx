    ...
    S.search = {
        categoriesMap: {
            ...
            baike: {
                ...
                'actionask': 'http://baike.1688.com/doc/add.html',
                'placeholder': '输入问题关键词'
            },
            pifa: {
        ...
        */
        ...
        basicChangeCategory: function(){
            ...
            $E.delegate( this.nodes.basicList, 'click', function(e){
                ...
                //for customized baike START
                if ( 'baike' === type ){
                    $D.addClass( nodes.customized, 'customized-baike' );
                    nodes.submit.innerHTML = '搜答案';
                } else {
                    $D.removeClass( nodes.customized, 'customized-baike' );
                    nodes.submit.innerHTML = '搜索';
                }
                //for customized baike END
        ...
        /**
         * click ask button
         * @usefor customized baike
         * */
        clickBaikeAsk: function(){
            var _this = this,
                nodes = this.nodes,
                form = document.createElement('form'),
                input = document.createElement('input');
            $D.addClass( form, 'fd-hide' );
            input.setAttribute( 'name', 'keywords' );
            $E.addListener( nodes.submitAsk, 'click', function(e){
                var kw, category, url, isAskUrl,
                    type = _this.getCurrentType(),
                    target = '_self';
                if ( 'baike' === type ){
                    category = _this.categoriesMap[type];
                    url = win.location.href.split('?')[0];
                    isAskUrl = url === category['actionask'];
                    kw = nodes.input.value.trim();
                    if ( ( kw !== '' ) && ( kw !== category['placeholder'] ) ){
                        input.value = kw;
                    } else {
                        if ( isAskUrl ){
                    ...
                    if ( 'undefined' !== typeof aliclick ){
                        aliclick( this, '?tracelog=baike_ask_syny01' );
                    }
                    form.setAttribute( 'action', category['actionask'] );
                    form.setAttribute( 'target', target );
                    form.appendChild(input);
                    $('ali-search').appendChild(form);
                    form.submit();
                } else {
                    FD.common.goTo( _this.categoriesMap['baike']['actionask'], target );
                }
            } );
        },
        ...
        init: function(){
            this.nodes = {
                ...
                //for customized baike START
                customized: $$('fieldset.customized-baike')[0],
                submit: $('alisearch-submit'),
                submitAsk: $('alisearch-submit-ask'),
                //for customized baike END
                ext: null
            };
...
if (!FD.sys.Masthead) {
    ...
    FYE.onDOMReady(function(){
        ...
        var handlers = {
             ...
             */
            typeInit: function(){
                var categories = FD.common.toArray(searchCategory.children), links = FYS('a', searchType);
                //主类目
