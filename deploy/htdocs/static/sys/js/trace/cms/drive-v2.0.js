...
if (window.FE && (!FE.sys || !FE.sys.Dcms || !FE.sys.Dcms.Drive)){
    ...
    ;(function($, drive){
		...
		//esi出错代码如下:
	        //title='esi request time-out' data-url='http://dcms-test.china.alibaba.com/open/error.htm?path=/open/pos/yqg_err2.htm'
		//判断是否包含open/error.htm?path=，有则是esi解析出错,如果后台请求参数变了，此也会受影响
		if(url.indexOf("open/error.htm?path=") > -1) {
		    esiFailure(that, '数据内容为空，请联系管理员',url);
		    return;
		}
		//js方式请求投放位
                $.ajax({
                    ...
                    success:function(o){
                        if (o&&o.content){
                            ...
                            if (error && $.isFunction(error)){
                                ...
                                failure(that, '数据内容为空，请联系管理员',url);
                            }
                        }
                    },
                    error:function(){
	...
	function getPosFromUrl(url){
        ...
        function failure(el, html, url){
            //提取url后缀作为投放位
	    var position = getPosFromUrl(url),content;
	    if(position) {
		//已预埋容错模板
                if ($("#pos_error_" + position)){
		    content = $.trim($("#pos_error_" + position).html());	
		}
		if(content) {
		    insertHtml.init();
                    var objContent = insertHtml.handle('<div>' + content+'</div>', el, true);
                    insertHtml.reset();
