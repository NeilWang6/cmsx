 ...
 * @update 2012.04.12 yanqg  for 增加页面预埋错误模板处理，直接从页面上取得容错模板
 *
...
if (window.FD && (!FD.sys || !FD.sys.Dcms || !FD.sys.Dcms.Drive)){
    ...
    (function(w, drive){
        ...
        function request(els, callback){
            ...
            $D.batch(els, function(el){
                ...
                //esi出错代码如下:
	        //title='esi request time-out' data-url='http://dcms.1688.com/open/error.htm?path=/open/pos/yqg_err2.htm'
		//判断是否包含open/error.htm?path=，有则是esi解析出错,如果后台请求参数变了，此也会受影响
		if(url.indexOf("open/error.htm?path=") > -1) {
		    esiFailure(el, '数据内容为空，请联系管理员',url);
		    return;
		}

                var deletess = $D.getAttribute(el, 'data-deletess'),
                param = {},
                configs = {
 	...
 	/**
         * 从url获取投放位调用名称
         * url : 请求出错的url,用于出错打点和寻找预埋的错误模板
         */
	function getPosFromUrl(url){
	    var patt = new RegExp("\/([^\/]+)\.htm$","g");
	    var result;
         ...
         */
        function failure(el, html,url){
            if (!(el || html)){ return; }
	    var position = getPosFromUrl(url);
	    if(position) {
		//已预埋容错模板
		var errTmlContent = document.getElementById("pos_error_" + position + "");
		if(errTmlContent && errTmlContent.innerHTML) {
		    insertHtml.init();
                    var objHtml = insertHtml.handle('<div>' + errTmlContent.innerHTML+ '</div>', el, true);
                    insertHtml.reset();
		    iposerror(position);
		    return;
		}
	...
	/**
         ...
         * url : 请求出错的url,用于出错打点和寻找预埋的错误模板
         */
        function esiFailure(el, html, url){
	   failure(el,html,url);
        }
	...
	/**
         * 投放位出错打点
         */
        function iposerror(params){
            var script = document.createElement('script');
            insertHead(script);
            script.setAttribute('type', 'text/javascript');
            script.src = 'http://dcms.cn.1688.com/open/pos_error.htm?info='+params;
        }
        /**
         * 将元素插入到 head 中
