...
!(function($){
	...
	var configs = {
		dataField:    'data-mod-config',	//server端的数据通过这个属性埋入到模块对应dom节点中，框架会负责取出节点中数据并传给模块的this.config.data变量。
											//例如：<div data-mod-config='{"name":"title","url":"http://exodus.1688.com/offer/json/validate.htm"}'/></div>
    ...
    $.extend(AppEntity.prototype, {
		...
		*/		
		...
		_createInstance: function(moduleId, opt){
			...
			var opt = this._getModuleConfig(moduleId, opt, sb);
		...
		_getModuleConfig:function(moduleId, opt, sb){
			...
			if(!this.docDetect){
				try{
					...
					if(doc.length>0){
						...
						if(data && data.trim()){
							data = data.replace(/\\'/g, "'");
							opt.global = $.parseJSON(data);
							sb.set(this.config.docField, opt.global);
						}
					}
				}catch(e){
				}
				...
				opt.global = sb.get(this.config.docField);
