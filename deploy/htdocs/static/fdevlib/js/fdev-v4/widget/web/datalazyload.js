 ...
 * @update : qhwa 2012-05-21 修复在IE8下图片大小不重置的BUG
 * @update : jianping.shenjp 2013-1-25 bind方法优化，防止出现bind方法注册较迟，滚动事件已注销，导致bind的方法不执行的情况发生
 */
('datalazyload' in FE.util) ||
(function($, Util){
    var WIN = window, DOC = document, IMGSRCNAME = 'data-lazyload-src', FNCLASSNAME = 'lazyload-fn', FNATTRIBUTE = 'data-lazyload-fn-body', TEXTAREACLASSNAME = 'lazyload-textarea', datalazyload = function(){
        var _defaultThreshold = 200, _threshold = 0, options = {}, _isRunning = false, _viewportHeight = 0, _scollBody = $(WIN),        //资源池
        resourcePool = {
            img: [],
            fn: [],
            textarea: []
        ...
        _collect = function(container){
            var imgs = $('img[' + IMGSRCNAME + ']', container).toArray(), fns = $('.' + FNCLASSNAME, container).toArray(), textareas = $('.' + TEXTAREACLASSNAME, container).toArray();
        ...
        _checkPosition = function(el){
            var ret = false, currentScrollTop = $(DOC).scrollTop(), benchmark = currentScrollTop + _viewportHeight + _threshold, currentOffsetTop = $(el).offset().top;
