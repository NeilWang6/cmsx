 ...
 * @update terence.wangt 2013.06.17 使用widgetBaseClass替代硬编码的ui-dialog，使得组件支持对样式的扩展。
 */
('dialog' in jQuery.fn) ||
(function($, undefined){
    ...
    $.widget('ui.dialog', {
        ...
        _create: function(){
            ...
            if (o.modal) {
                ...
                contentLyr.css('z-index', $dialog.zIndex++).bind('keydown.' + self.widgetBaseClass, function(event){
            ...
            if (ie6) {
                ...
                //bind window event
                if (o.fixed) {
                    $(window).bind('scroll.' + self.widgetBaseClass, $.proxy(function(event){
                        this._offsetContent();
                    }, self));
                }
            }
            ...
            if (o.center) {
                ...
                //bingd window event
                if (o.fixed) {
                    $(window).bind('resize.' + self.widgetBaseClass, $.proxy(self, '_center'));
                }
            }
            else {
                self._offsetContent();
        ...
        destroy: function(){
            ...
            $(window).unbind('.' + self.widgetBaseClass);
            contentLyr.unbind('.' + self.widgetBaseClass).remove();
            self._trigger('close');
    ...
    $.extend($.ui.dialog.overlay, {
        ...
        create: function(dialog){
            ...
            if (modal) {
                ...
                doc.bind('keydown.' + dialog.widgetBaseClass +'-overlay,keypress.' + dialog.widgetBaseClass+ '-overlay', function(event){
                ...
                modal = (self.modal = $('<div>', {
                    'class':  dialog.widgetBaseClass + '-overlay',
                ...
                if (ie6) {
                    $(window).bind('resize.' + dialog.widgetBaseClass +'-overlay', function(){
                        modal.css({
                            width: body.width(),
                            height: doc.height()
                        });
        ...
        _destroy: function(dialog){
            var self = this;
            //先去除事件
            $(document).unbind('.' + dialog.widgetBaseClass +'-overlay');
            if (ie6) {
                $(window).unbind('.' + dialog.widgetBaseClass + '-overlay');
            }
            self.modal.remove();
            self.modal = null;
        }
