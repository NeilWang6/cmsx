...
(function($, undefined){
    ...
    $.extend($.ui.datepicker.TimeSelector.prototype, {
        ...
        _init: function(ft){
            var self = this;
			var timeCls = self.father.widgetBaseClass +'-time';
            ft.html('').append(self.ctime);
            ft.append(self.button);
            ft.append(self.clearBtn);
            self.render();
            self.popupannel.bind('click.'+ timeCls, function(e){
                var el = $(e.target);
                if (el.hasClass('x')) {//关闭
                    self.hidePopup();
                }
            ...
            //确定的动作
            self.button.bind('click.'+ timeCls, function(e){
                //初始化读取父框的date
            ...
            self.clearBtn.bind('click.'+ timeCls, function(e){
            ...
            //TODO 考虑是否去掉
            self.ctime.bind('keyup.'+ timeCls, function(e){
                var KC = $.ui.keyCode;
            ...
            //上的箭头动作
            $('button.u', self.ctime).bind('click.'+ timeCls, function(){
                self.hidePopup();
                self.add();
            });
            //下的箭头动作
            $('button.d', self.ctime).bind('click.'+ timeCls, function(){
                self.hidePopup();
                self.minus();
            });
            //弹出选择小时
            $('span.h', self.ctime).bind('click.'+ timeCls, function(){
                var in_str = self.parseSubHtml(self.h_a);
                self.status = 'h';
                self.showPopup(in_str);
            });
            //弹出选择分钟
            $('span.m', self.ctime).bind('click.'+ timeCls, function(){
                var in_str = self.parseSubHtml(self.m_a);
                self.status = 'm';
                self.showPopup(in_str);
            });
            //弹出选择秒
            $('span.s', self.ctime).bind('click.'+ timeCls, function(){
