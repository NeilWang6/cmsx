 ...
 * @update 2012.07.18 shiwei.dengsw 使配置项selected支持负数，当为负数时，初始化时的当前触点将是随机的
 * 
    ...
    $.widget('ui.tabs', {
        ...
        _create: function() {
            ...
            if (defaultIdx < 0) { // tabs第一页随机选择 added by shiwei.dengsw 2012.07.18
                ...
                defaultIdx = Math.floor(Math.random() * (max - min + 1)) + min;
            }
            self._setTab(defaultIdx, null, true);
            //自动播放
            self._autoPlay(1);
        ...
        reset: function(){
            ...
            if (this.options.effect==='scroll'){   // && this._getScrollBoxes().length===0
                this._setEffectStyle(true);
                if (this._getScrollBoxes().length===0){
                    this.prev();
                }
            }
        },
