 ...
 * @update 2012.05.28 hongss 优化在滚动状态下的reset功能，但是当scrollType为loop时无法实现真正的reset功能
 * @update 2012.12.03 hongss 修复在滚动状态下的reset后，this.boxes未被reset的问题
 * @update 2013.02.18 raywu  优化渐进渐出动画，引入fx-core，从基础的swing效果改为easeInQuad，效果更柔和点
 */
 ...
 ;(function($, undefined){
     ...
     $.extend($.ui.tabs.prototype, {
         ...
         /**
          * @methed _resetScroll  重置滚动效果的style设置
          * @update add by hongss on 2012.05.28 
          */
         /*_resetScroll: function(){
            this.prev();
            this._setEffectStyle(true);
         },*/
         /**
         ...
         _effectFade: function(idx) {
             ...
             self.boxes.hide();
             $(boxes).fadeIn(self.options.speed,'easeInQuad', function(){
                //自定义事件 show
                self._trigger('show', null, {index:idx, box:boxes});
                self._autoPlay(1);
             });
          ...
          */
         _setEffectStyle: function(isReset){
             if (this.options.effect==='scroll'){
                 var initOffset = this._getPerLength() * this.index,
                 parent = this._getBoxesParent().parent();
             
                 if (this._isHorizontal()){
                     this._setOffset('width', isReset);
                     parent.scrollLeft(initOffset);
                 } else {
                     this._setOffset('height', isReset);
                     parent.scrollTop(initOffset);
                 }
                 
                 this._setSubLength();
                 this._setPerItem();
                 this._setBoxes(isReset);
          ...
          */
         _setOffset: function(cssName, isReset){
             var boxes = this._getPrimalBoxes(isReset),  
             teamsLength = this._getTeamCount(isReset) * this._getPerLength(),
          ...
          */
         _getPrimalBoxes: function(isEdit){
             var boxes = this.element.find(this.options.boxSelector);
             this._getPrimalBoxes = function(isEdit){ 
                 if (isEdit===true){
                     return this.element.find(this.options.boxSelector);
                 }
                 return boxes; 
             }
             return boxes;
          ...
          */
         _getTeamCount: function(isReset){
             var boxes = this._getPrimalBoxes(isReset),
          ...
          */
         _setBoxes: function(isReset){
             if (this.options.perItem > 1){
                 var perItem = this.options.perItem,
                 boxes = this._getPrimalBoxes(isReset),
                 n = this._getTeamCount(isReset),
                 arrBoxes = [];
