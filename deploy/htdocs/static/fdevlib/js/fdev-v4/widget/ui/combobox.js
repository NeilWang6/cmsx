 ...
 * @update terence.wangt 2013.06.17 使用widgetBaseClass替代硬编码的ui-combobox，使得组件支持对样式的扩展。
 */
...
(function($, undefined){
    ...
    $.widget('ui.combobox', {
        options: {
            ...
            triggerText: '',
            //请注意在这个方法中需要做的事情，
            tpl: function(data, preCls){
                var ul = $('<ul>'), i, j;
                for (i = 0, j = data.length; i < j; i++) {
                    var item = data[i], li = $('<li>').addClass( preCls + '-item').data('item', item);
        ...
        _create: function(){
            ...
            elem.addClass( self.widgetBaseClass ).append(html.join(''));
            
            //创建下拉Panel 
            panel = self.panel = $('<div>', {
                'class': self.widgetBaseClass + '-panel',
                css: {
                    display: 'none',
                    zIndex: o.zIndex
                },
        ...
        _buildEvent: function(){
            ...
            //多选模式下
            if (o.multiple) {
                self.result.delegate('a.'+ self.widgetBaseClass +'-remove', 'click.combobox', function(e){
                ...
                self.result.bind({
                    'keyup.combobox': function(){
                        var custom = true;
                        $(itemNode + '.'+ self.widgetBaseClass +'-item', self.list).removeClass( self.widgetBaseClass +'-selected').each(function(i, node){
            ...
            self.list.delegate(itemNode + '.'+ self.widgetBaseClass +'-item', {
                'hover.combobox': function(){
                    $(this).toggleClass( self.widgetBaseClass + "-hover");
                },
                'click.combobox': function(e){
                    ...
                    if (item) {
                        ...
                        if (o.multiple) {
                            ...
                            if (node.hasClass( self.widgetBaseClass + '-selected')) {
                                self.remove(value);
                            } else {
        ...
        _mousedownHandler: function(e){
            if ($(e.target).closest('.'+ e.data.widgetBaseClass)[0] !== e.data.element[0]) {
                e.data.panel.hide();
            }
        },
        /**
        ...
        _listRender: function(data){
            ...
            self.value = [];
            self.data = data;
            self.list.html(o.tpl(data, self.widgetBaseClass));
            self._resultInit();
            if (o.maxHeight) {
                self.panel.css({
                    width: '',
        ...
        _resultInit: function(){
            ...
            var self = this, 
				o = self.options, 
				itemNode = o.itemNode,
				sels = $(itemNode + '.'+ self.widgetBaseClass +'-selected', self.list);
			//format
            //假如有默认值
            if (o.multiple) {
                self.result.empty();
                ...
                if (i > -1) {
                    ...
                    if (!node.hasClass( self.widgetBaseClass + '-selected')) {
                        if (o.multiple) {
                            ...
                            $(itemNode + '.'+ self.widgetBaseClass +'-item', self.list).removeClass( self.widgetBaseClass + '-selected');
                            //result
                            self.result.val(o.resultTpl(item));
                            self.field.val(item.value);
                        }
                        node.addClass( self.widgetBaseClass + '-selected');
        ...
        remove: function(value, node){
            ...
            $(itemNode + ':data(item)', self.list).each(function(i, node){
                ...
                if (i > -1) {
                    if (node.hasClass( self.widgetBaseClass + '-selected')) {
                        //change
                        node.removeClass( self.widgetBaseClass + '-selected');
