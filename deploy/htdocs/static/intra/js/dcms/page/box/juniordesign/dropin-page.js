...
;(function($, D, undefined) {
	...
	function handleLoad(el) {
        ...
        D.box.editor.insertDecoration(doc);
        D.box.editor.initSelectModule(doc, self.config.dropArea);
        D.box.editor.initMenuHandler(doc, self);
	...
	var DropInPage = function(config) {
		...
		this.insertIframe = function() {
			...
			var config = this.config, pageUrl = config.pageUrl, strParam = '';
            
            //modify by hongss on 2013.08.20 for 根据栅格获取不同的页面原型
            if (config.pageUrlParam){
                strParam = '?' + config.pageUrlParam;
            }
            var iframe = $('<iframe id="' + config.iframeName + '" class="dcms-box-main" src="' + pageUrl + strParam +'" />');
	...
	DropInPage.prototype = {
		...
		_hideAll : function() {
            ...
            /*add by hongss on 2012.08.09 for 新交互*/
            var layoutCover = $(D.box.editor.config.SELECTOR_LAYOUT_COVER, this.iframeDoc),
                normalCover = $(D.box.editor.config.SELECTOR_MODULE_COVER, this.iframeDoc);
            D.HighLight.hideLight(layoutCover);
            D.HighLight.hideLight(normalCover);
            D.HighLight.removeLightClassName($('.crazy-box-layout', this.iframeDoc), D.box.editor.config.CLASS_LAYOUT_HIGHT_LIGHT);
            D.HighLight.removeLightClassName($('.crazy-box-module', this.iframeDoc), D.box.editor.config.CLASS_EDIT_HIGHT_LIGHT);
        },
		/**
		...
		_enableDelLevel : function(elem) {
			...
			if(type === 'layout' || type === 'row' || type === 'cell') {// || type==='module'
				...
				if(options && D.BoxTools.parseOptions(options, ['ability', 'delete', 'enable']) === "true") {
					result['enable'] = true;
					result['type'] = type;
				} else if(currentElem.hasClass('crazy-box-banner')){
                    result['enable'] = true;
					result['type'] = type;
                } else {
					result['enable'] = false;
                ...
                /*if (currentElem.siblings('.crazy-box-'+type).length <= 0){
                    result['enable'] = false;
                } else {
                    result['enable'] = true;
                }*/
		...
		_delBtnListener : function() {
			...
			$('.' + self.config.delButton, self.iframeDoc).live('mousedown', function(e) {
				...
				self._deleteFn.call(this, currentElem);
			});
        ...
        _deleteFn: function(currentElem) {
            var self = this,
                result = self._enableDelLevel(currentElem), elemInfo= currentElem.data('eleminfo'),delBtn = self.delBtn = $(this);
            if(self.state === 'copy') {//取消复制
                self._cancelCopy();
            }
		...
		_clickPagePackage : function() {
            ...
            self.iframeDoc.bind('click', function(e) {
				...
				var target = $(e.target);
				if(self.editTextarea.css('display') !== 'none' && target.closest('.' + self.config.defineCell).length === 0 && target.closest(self.config.editTextarea).length === 0 && !target.closest('.edit-define')[0]) {
			...
			self.dropArea.live('mousedown', function(e) {
				if (!D.BoxTools.isNeedModify($(e.target))){
                    return;
                }
			...
			var pretarget;
			self.dropArea.live('click.editcontent', function(e) {
				if (!D.BoxTools.isNeedModify($(e.target))){
                    return;
                }
                e.preventDefault();
				...
				if(self.chooseLevel === 'label') {
					...
					if(D.YunYing && D.YunYing.isYunYing) {
						...
						if(typeof ancestor.data('boxconfig') !=='undefined' || typeof ancestor.data(D.box.editor.config.ATTR_DATA_DYNAMIC) !== 'undefined'){
							if (D.BoxTools.isNeedModify(target)){
                                //前端源码组件事件监听
		...
		_setDefineCodeEdit : function(target, editTextarea) {
			var dsdynamic = $.trim(target.attr('data-dsdynamic')), h = w = '100px', 
                nH, nW;
			/*if(parseInt(target.children().css('height')) > 100) {
				h = target.children().css('height');
			}*/
            if ((nH = parseInt($(window).height()))>100){
                h = nH - 160;  //110
            }
            if ((nW = parseInt($(window).width()))>100){
                w = nW - 260; //52
            }
			//D.HighLight.showLight(editTextarea, target);
			editTextarea.css('height', h);
			editTextarea.css('width', w);
            
            editTextarea.data(D.box.editor.TRANSPORT_DATA_ELEM, target);
			/*var top = parseInt(editTextarea.css('top')),
                left = parseInt(editTextarea.css('left'));
			editTextarea.css('top', top + 30);
			editTextarea.css('left', left + 30);
            editTextarea.css('width', parseInt(target.outerWidth())-60);*/
            
			if(D.YunYing && D.YunYing.isYunYing) {  //需求更改为只有vm类型的自定义控件
				if (dsdynamic){
		 ...
		 * @param target jQuery对象，目标元素
		 */
		_setDefineCodeView : function(editTextarea, el) {
			...
			el = el || editTextarea.data(D.DropInPage.CONSTANTS.TRANSPORT_DATA_ELEM);
			
            if(!el) {
				return;
			}
            /*add by hongss on 2013.08.08 for 新遮罩修改*/
            /*var editCover = $(D.box.editor.config.SELECTOR_DATAEDIT_COVER, this.iframeDoc),
                focuseEl = $('.edit-define', editCover);
            //setTimeout(function(){
                D.HighLight.hideLight(editCover);
            //}, 10);
            focuseEl.removeClass('current').text('编辑源代码');*/
            //需求更改为只有vm类型的自定义控件
