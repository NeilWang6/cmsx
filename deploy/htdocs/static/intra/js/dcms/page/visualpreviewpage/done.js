 ...
 */
 ...
 ;(function($, D){
    ...
    arrangeOperate = $('operate.js-operate-arrange'),
    ...
    readyFun = [
        ...
        function(){
            coverObj = new D.Cover({
                ...
                hide: function(el){
                    var operate = el.children('operate');
                    if ((operate.length===0)){
                        var _operateArray = getOperate(el);
						// 将对象存入到doc中
						for (var i=0;i<_operateArray.length;i++){
							var _operatorTmp = _operateArray[i].clone();
                        	el.append(_operatorTmp);
						}
                    }
					var operate = el.children('operate');
            ...
            function getOperate(el){
                ...
                if (el[0].tagName.toUpperCase()===POSITION_TAG){
                    //operate[0] = positionOperate;
					operate.push(positionOperate);
                } else {
					// 如果即有排期又有模板，返回两个
                    if(el.attr('blockid')) {
						operate.push(arrangeOperate);
		 				//operate = arrangeOperate;
				    } 
					if (el.attr('templatecode')){
						operate.push(templateOperate);
        ...
        function(){
            ...
            saveBtn.click(function(e){
                ...
                saveCode(container, 'event_submit_do_save_page');
        ...
        function(){
		    ...
		    blockId = container.attr('blockid'),
				...
				if (arrangeEditBtn.length>0){
					var _url = D.domain+'/page/arrange/arrange_block_group.html?action=arrange_action&event_submit_do_search_block_group=true&blockId='+blockId;
					// 把模板名也设置上去
					_url +="&templateCode=" +templateCode;
                    arrangeEditBtn.attr('href', _url);
                }
            });
        },
        /**
         ...
         */
        function(){
            var expiredOfferId=$('#dcms-expired-offer-id');
            if (expiredOfferId && expiredOfferId.val()){
                var cancelBtn = $('.js-cancel-btn', confirmEl);
                cancelBtn.hide();
                D.Message.confirm(confirmEl, {
                    title: '温馨提示',
                    msg: '页面中的Offer已过期。对应的ID分别是：<div>'+expiredOfferId.val()+'</div>',
                    enter: function(){
                        cancelBtn.show();
                    }
                  });
    ...
    function setOperateOffset(el, operate){
		...
		// 是数组，支持多个
		operate.each(function(index, obj) {
            var _self = $(obj);
			_self.css({'top': (top-OPERATE_DIV_HEIGHT)+'px', 'left': left+'px'});
			// 下一轮的left要在当前操作对象的宽度基础上加
			left += _self.width() + 10;
        });
