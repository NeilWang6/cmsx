(function($,D,undefined){
	...
	var PageOperateHistory = {
		init:function(editPage){
			var self = this,iframeBody = editPage.iframeBody;
			...
			$('<li/>').html('<a href="#" class="dcms-box-hand  dcms-box-abtn undo disbaled" title="copy" id="operate-undo">撤销</a>').appendTo(appendNode);
			
			$('<li/>').html('<a href="#" class="dcms-box-hand  dcms-box-abtn redo disbaled" title="copy" id="operate-redo">恢复</a>').appendTo(appendNode);
			...
			$('#operate-redo').bind('click',function(e){
                ...
                editPage&&editPage.hideAllSingers();
				var self = this;
			...
			$(win).bind('PageOperateHistory.undo',function(evnet,param){
				...
				for(var i=data.length-1;i>=0;i--){
					...
					el = D.BoxTools.getElem(undoItem.node, iframeBody || editPage.iframeBody);
			...
			$(win).bind('PageOperateHistory.redo',function(event,param){
				...
				for(var i=0,len = data.length;i<len;i++){
					...
					el = D.BoxTools.getElem(redoItem.node, iframeBody || editPage.iframeBody);
        ...
        getNodeInfo: function(opts){
            ...
            switch (opts['editType']){
                ...
                case 'delete':
                    ...
                    nodeInfo['fn'] = D.EditContent.stepDel;
