...
;(function($, D, undefined) {
	D.BoxTools = {
		/**
		 *  判断组件类型，新规范组件(option),原规范组件(labeledit)
		 */
        getModuleType: function(elem){
            //确保是组件
            if (elem.hasClass('crazy-box-module')){
                if (elem.data(D.box.editor.config.ELEMENT_DATA_BOX_CONFIG) || elem.data(D.box.editor.config.ATTR_DATA_DYNAMIC)){
                    //新规范组件
                    return D.box.editor.config.MODULE_TYPE_OPTION;
                }
                return D.box.editor.config.MODULE_TYPE_LABELEDIT;
            }
        },
        
        /**
		 *  是否允许修改，所有不允许修改的情况都放在此方法内;其实是对元素是否为组件自身元素的判断
		 */
        isNeedModify: function(el){
            if (!el[0]){
                el = $(el);
            }
            //如果是错误信息提示内容内的元素不允许修改
            if (el.closest('.'+D.box.editor.config.CLASS_ERROR_MESSAGE)[0]){
                return false;
            }
            return true;
        },
		 ...
		 *  对自定义代码中的<及>做转义处理
		 */
		handleDynamic : function(html) {
			var req = /(data-dsdynamic=)"([^"]*)"/gm;
			return (html&&html.replace(req, function(s, attr, value) {
					return attr + '"' + value.replace(/\</gm, "&lt;").replace(/\>/gm, "&gt;") + '"';
				}));
		},
		/**
		 ...
		 * pingchun.yupc
		 */
		loadStyles : function(url, parent) {
			var _url = [], _parent, _fragment = document.createDocumentFragment();
		 ...
		 * pingchun.yupc
		 */
		loadScripts : function(url, parent) {
			var _url = [], _parent, _fragment = document.createDocumentFragment();
			if( typeof url === 'string') {
				_url.push(url);
			}
			...
			if( parent instanceof jQuery||parent instanceof dcms) {
				_parent = parent[0];
			} else {
				_parent = parent;
			}
		...
		addEditAttr : function(htmlCode) {
			...
			$module.each(function(index, module) {
				var _self = $(module);
				self.addBoxOptions(_self, 'ability', {
					"editAttr" : [{
						"key" : "id",
						"name" : "ID"
					}]
		...
		addBoxOptions : function($arg, type, o) {
			var _boxOptions = $arg.data(D.bottomAttr.CONSTANTS['boxOptions']), _addCssBoxOptions = function($arg, type, o, cssObj) {
				...
				var isEx = false, abilityOptions = _boxOptions['ability'], isEditAttr = false, editAttr = [], editAttred = [];
				if(abilityObj) {
					...
					if(!isEx) {
						...
						if(isEditAttr) {
							...
							editAttr = o['editAttr'];
							//标签上已有的属性
							$.merge(editAttred, abilityObj['editAttr']);

							var aa = [];
							if(editAttr && editAttr.length) {
								for(var n = 0; n < editAttr.length; n++) {
									var _temp = editAttr[n];
									if(!checkIsIn(_temp.key, editAttred)) {
										abilityObj['editAttr'].push(_temp);
									}
								}
			...
			}, checkIsIn = function(key, editAttred) {
				if(key && editAttred && editAttred.length) {
					for(var i = 0; i < editAttred.length; i++) {
						var temp = editAttred[i];
						if(key === temp.key) {
							return true;
						}
					}
