 ...
 * @userfor 编辑内容模块
 * @date  2012-02-10
 * @modify  by hongss on 2013.01.08 for add next new functions (moveUp, moveDown, copyOnce)
 */
    ...
    D.EditContent = {
        ...
        editContent: function(opts){
            ...
            if (opts['isEdit']!==false){
                nodeInfo['redo'] = D.PageOperateHistory&&D.PageOperateHistory.getNodeInfo({
                    'execEl': opts['elem'],
                    'editType': D.EditContent.editContent,
                    'args': newOpts
                });
                
                nodeInfo['undo'] = D.PageOperateHistory&&D.PageOperateHistory.getNodeInfo({
                    'execEl': opts['elem'],
                    'editType': D.EditContent.editContent,
                    'args': oldOpts
                });
        ...
        /**
         * @methed moveUp 上移，目前只考虑cell下标签的移动，即不考虑带style的情况
         *         add by hongss on 2013.01.08  
         * @param opts 配置项  {elem:el(jQuery对象), doc:iframeDoc, isEdit:true/false}
         *                  isEdit:当true时说明需要记录到“回撤”数据中去
         */
        moveUp: function(opts){
            var elem = opts['elem'], nodeInfo,
                prevEl = elem[0] && elem.prev();
            if (prevEl[0]){
                if (opts['isEdit']===true){
                    D.box.editor.operate.moveToReplace(elem, prevEl, 'label', opts['doc']);
                } else {
                    prevEl.before(elem);
                }
            }
        ...
        /**
         * @methed moveDown 下移，目前只考虑cell下标签的移动，即不考虑带style的情况
         *         add by hongss on 2013.01.08  
         * @param opts 配置项  {elem:el(jQuery对象), doc:iframeDoc, isEdit:true/false}
         *                  isEdit:当true时说明需要记录到“回撤”数据中去
         */
        moveDown: function(opts){
            var elem = opts['elem'], nodeInfo,
                nextEl = elem[0] && elem.next();
            if (nextEl[0]){
                if (opts['isEdit']===true){
                    D.box.editor.operate.moveToReplace(elem, nextEl, 'label', opts['doc']);
                } else {
                    nextEl.after(elem);
                }
            }
        ...
        /**
         * @methed copyOnce 单次复制，目前只考虑cell下标签的移动，即不考虑带style的情况
         *         add by hongss on 2013.01.08  
         * @param opts 配置项  {elem:el(jQuery对象), doc:iframeDoc, isEdit:true/false}
         *                  isEdit:当true时说明需要记录到“回撤”数据中去
         */
        copyOnce: function(opts){
            var elem = opts['elem'];
            if (!elem[0]){ return; }
            
            if (opts['isEdit']===true){
                D.box.editor.operate.copyToNext(elem, opts['doc']);
            } else {
        ...
        _getStyleEl: function(widgetEl, cellClassName){
            var styleEl = widgetEl.siblings('style[data-for='+cellClassName+']').not('[type="text/less"], [data-src="less"]');
