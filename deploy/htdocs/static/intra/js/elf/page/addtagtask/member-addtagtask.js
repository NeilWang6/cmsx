...
;(function($, E){
    ...
    var domain = location.protocol + '//' + location.host,
		...
		//支付金额验证
		function(){
            $('.js-change-validate-007').change(function(){
                var min_007 = Number($.trim($('#pay_ord_amt_1d_007_min').val()));;
				var max_007 = Number($.trim($('#pay_ord_amt_1d_007_max').val()));
				var tip = $('.validator-tip-007');
                if(min_007 > 0 && max_007 > 0 && min_007 > max_007){
                    tip.text("最大值不能小于最小值");
                    tip.addClass('validator-error-pay');
					$('#form-add-task')[0].onsubmit = function(){
						return false;
					}
                }else{
					tip.text("");
                    tip.removeClass('validator-error-pay');
					$('#form-add-task')[0].onsubmit = function(){
						return true;
					}
                }
            });
			$('.js-change-validate-008').change(function(){
                var min_008 = Number($.trim($('#pay_ord_amt_1w_008_min').val()));;
				var max_008 = Number($.trim($('#pay_ord_amt_1w_008_max').val()));
				var tip = $('.validator-tip-008');
                if(min_008 > 0 && max_008 > 0 && min_008 > max_008){
                    tip.text("最大值不能小于最小值");
                    tip.addClass('validator-error-pay');
					$('#form-add-task')[0].onsubmit = function(){
						return false;
